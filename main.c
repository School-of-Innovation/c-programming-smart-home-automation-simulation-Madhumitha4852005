...
void houseStatusSummary(int lights[], int temp[], int motion[], int locks[], int rooms);
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 5: houseStatusSummary(lights, temp, motion, locks, rooms); break;
...
void initializeSystem(int lights[], int temp[], int motion[], int locks[], int rooms) {
    for (int i = 0; i < rooms; i++) {
        lights[i] = 0; // OFF
        temp[i] = 22 + (rand() % 6); // Random temp between 22-27
